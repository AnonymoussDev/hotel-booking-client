...
const Reservation = ({ expectedCheckIn, expectedCheckOut, num, type, room, reservation, services, step }) => {
    ...
    useEffect(() => {
        ...
        if (room.sale?.salePercent) {
            setDiscountPrice(room.price - (room.price * room.sale?.salePercent) / 100);
        }

        console.log('add service', services);
        if (services?.length > 0) {
            let newPrice = price + services[services.length - 1].price;
            let newDiscountPrice = discountPrice + services[services.length - 1].price;
            setPrice(newPrice);
